...
Track the pieces in a todo list and keep looping until the critic picks ours blind.

Run-to-completion contract: never end your turn while a todo is still pending. If interrupted, read the todos and resume from the first pending one. Commit after each piece passes. The only acceptable stop is every check green and everything committed - never a round count, never a checkpoint.
...
`/loop` and `ultracode` are Claude Code features. Most other agents have neither - and an agent that reads instructions it cannot follow loses all pacing and stops at the first natural checkpoint, usually after two or three tasks.

The default template therefore contains no agent-specific commands. The todo list plus the run-to-completion contract provide pacing on any agent; subagent fan-out is plain instruction. Only if you know the target is Claude Code may you append "/loop on each piece" and "ultracode" as convenience lines - never as the only pacing mechanism.
...
- **No stop contract.** Without "never end your turn while a todo is pending", agents stop at the first natural checkpoint after a few tasks - the most common way loops quietly die.
- **Agent-specific commands on the wrong agent.** Pasting `/loop` or `ultracode` into an agent that lacks them removes all pacing; the run stalls and the model wraps up early.
